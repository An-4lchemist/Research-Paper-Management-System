...
def submit_getLitSurvey(req):
    print('sending getting literature survey')
    response = requests.post("http://localhost:3002/GetLitSurveys/", json=req)

    print('response for get literature')
    res_json = response.json()
    print(res_json)
    status, res_error = check_res_default(res_json)
    return status, res_error, res_json
